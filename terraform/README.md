...
| read_sharedservices_networking_terraform_state_policy_description | The description to associate with the IAM policy that allows read-only access to the Shared Services networking state in the S3 bucket where Terraform state is stored. | `string` | `Allows read-only access to the Shared Services networking state in the S3 bucket where Terraform state is stored.` | no |
| read_sharedservices_networking_terraform_state_policy_name | The name to assign the IAM policy that allows read-only access to the Shared Services networking state in the S3 bucket where Terraform state is stored. | `string` | `ReadSharedServicesNetworkingTerraformState` | no |
