...
variable "account_provisioners_group_membership_name" {
  description = "The name to associate with the membership of the IAM group allowed to assume the role with sufficient permissions to provision the DNS account."
  default     = "dns_account_provisioners_membership"
}
...
variable "assume_provisionaccount_policy_description" {
  ...
  default     = "Allow sufficient permissions to provision all AWS resources in the DNS account."
}
